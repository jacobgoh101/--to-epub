...
export const generateEpub = (obj) => {
    return new Promise((resolve, reject) => {
        ...
        // epub building
        const epubMaker = new Streampub({title})
        epubMaker.setAuthor(author)
        const fsStream = epubMaker.pipe(fs.createWriteStream(epubFileLocation))
        const chapterLength = content.length;
        content.map((cont, index) => {
            epubMaker.write(Streampub.newChapter(cont.title, `<h3>${cont.title}</h3><br/>${cont.data}`));
